...
async function runTests() {
    ...
    try {
        ...
        // 9. Executive Fetching Advance Payment Orders
        console.log("\n👉 Test 9: Executive Fetching Advance Payment Orders");
        const execPendingPaymentCheck = await request("/executive/orders/pending_payment", {
            headers: execHeaders,
        });
        assert(execPendingPaymentCheck.status === 200, "Pending payment orders returns 200");
        assert(execPendingPaymentCheck.data.orders.some((o) => o._id === orderId), "Order appears in advance payment list");
